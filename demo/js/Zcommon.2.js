...
function load_login(){
    ...
    Zframe.to('member/login',{back_url:back_url})
}
...
var Zpage = function (page_data){
    ...
    this.onReachBottom = page_data.onReachBottom||false;
    this.is_bottom = false;
    this.scroll_elem.addEventListener('scroll', function(e){
        //如果产生ajax请求有可能改变scrollTop，这里需要恢复
        if(_this.onScroll)_this.onScroll(this.scrollTop);
        if(_this.is_bottom){
            ...
            if (this.scrollTop >= this.scrollHeight - this.offsetHeight) {
                ...
                if(_this.onReachBottom)_this.onReachBottom(this.scrollTop);
            }
        }
    },true);
...
var Zvalidate = function(data,validate){
	for(var i in validate){
		if(isString(validate[i])){
			if(!data[i]){
				this.msg = validate[i];
				return validate[i];
			}
		}
		else if(isObject(validate[i])){
			if(validate[i].check == 'num'){
				data[i] = parseInt(data[i]);
				if(!data[i]){
					this.msg = validate[i].msg;
					return validate[i].msg;
				}
			}
		}
	}
